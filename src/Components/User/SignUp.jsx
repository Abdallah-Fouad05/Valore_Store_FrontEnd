...
function SignUp() {
    ...
    const handleSubmit = async() =>{
        ...
        try {
            const response = await fetch("https://localhost:7278/api/User/SignUp", {
                method: "POST",
                headers: {
            ...
            setLoginForm({ fullname:"",email: "", password: "",confirmPassword:"", });
            const islogged = true;
            if(islogged)
    ...
    {/* should we have save userID into local storage and use it into process that requires UserID => CartItem */}
    ...
    <div className='d-flex justify-content-center align-items-center vh-100'>
        <div className="position-relative rounded-3 p-4 w-100 m-2" style={{ maxWidth: "500px", backgroundColor : "#e5e5e5" }}>
            <form className="d-flex flex-column justify-content-center align-items-center h-100 ms-4 mt-4">
                <label htmlFor='fullname' className='w-100 mb-3 text-start'>
                    FullName: 
                    <input
                ...
                <label htmlFor='confirmPassword' className='w-100 mb-3 text-start'>
                    Confirm Password: 
                    <input 
                    className="form-control mt-2" 
